...
export default function CertificadosDocentesPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const res = await fetch('/api/admin/certificados-docentes');
        if (res.ok) {
  ...
  return (
    <>
      <style>{`
        ...
        .page-kicker { font-size: 0.68rem; font-weight: 800; letter-spacing: 2.5px; text-transform: uppercase; color: var(--accent); margin-bottom: 0.25rem; }
      ...
      <div className="page-header">
        <div>
          ...
          <h1 className="page-title">Certificados de Docentes</h1>
        </div>
      ...
      <Card padding="0">
        {loading ? (
          ...
          <EmptyState
            ...
            description="Aún no se han emitido certificados para docentes. Haz clic en 'Emitir Certificado' para comenzar."
          />
        ) : (
              ...
              <tbody>
                {certificates.map((cert) => (
                  <tr key={cert._id}>
                    ...
                    <td>{cert.datosCursoPrograma?.rolDocente}</td>
